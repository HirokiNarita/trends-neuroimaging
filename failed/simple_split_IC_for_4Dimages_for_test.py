...
def load_ic(file_name, num_ic):
    # load .mat
    f = h5py.File(config.input_base_path+'/fMRI_test/'+file_name,'r')
    data = f['SM_feature']
...
for num_ic in range(4,54):
    
    start_time = time.time()
    test_num_ic_matrix = coo_matrix((config.test_num_records, config.xyz), dtype=np.float64).tolil()
    
    for (row,file_name) in enumerate(tqdm(config.test_list)):
        test_num_ic_matrix[row,:] = load_ic(file_name, num_ic)
        gc.collect()
    sparse.save_npz(config.out_base_path+"/test/ic{}_matrix.npz".format(num_ic+1), test_num_ic_matrix.tocsr())
